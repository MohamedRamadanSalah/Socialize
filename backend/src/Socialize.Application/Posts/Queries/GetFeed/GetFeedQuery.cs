...
namespace Socialize.Application.Posts.Queries.GetFeed;

public record GetFeedQuery(Guid ViewerId, string? Cursor, int? Limit) : IRequest<CursorPage<PostDto>>;

public class GetFeedQueryHandler : IRequestHandler<GetFeedQuery, CursorPage<PostDto>>
{
    private readonly IApplicationDbContext _db;

    public GetFeedQueryHandler(IApplicationDbContext db)
    {
        _db = db;
    }

    public async Task<CursorPage<PostDto>> Handle(GetFeedQuery request, CancellationToken cancellationToken)
    {
        ...
        var followedAuthorIds = _db.Follows
            .Where(f => f.FollowerId == request.ViewerId)
            .Select(f => f.FolloweeId);

        var query = _db.Posts.Where(p => followedAuthorIds.Contains(p.AuthorId));
        ...
        var dtos = items.Select(p => ToDto(p, request.ViewerId)).ToList();
        return new CursorPage<PostDto>(dtos, nextCursor);
    }

    private static PostDto ToDto(Post p, Guid viewerId) => new(
        p.Id,
